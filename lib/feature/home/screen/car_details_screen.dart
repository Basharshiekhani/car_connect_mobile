...
class _CarDetailsScreenState extends State<CarDetailsScreen> {
  ...
  void getCar() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if ((response.body ?? "").isNotEmpty) {
        ...
        // Update liked state based on likesCount
        isLiked = (entity?.likesCount ?? 0) > 0;
      } else {
        setState(() {
          status = 2;
        });
  ...
  void addLike(carId, action) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      if ((response.body ?? "").isNotEmpty) {
        // Parse the response to get updated likes count
        var responseData = jsonDecode(response.body);
        if (responseData['likesCount'] != null) {
          setState(() {
            entity = entity?.copyWith(likesCount: responseData['likesCount']);
            isLiked = (responseData['likesCount'] ?? 0) > 0;
          });
        }
        getCar(); // Refresh car details
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  void addToFavorite(carId) async {
    try {
      ...
      print("URL: ${ApiPostUrl.addFavorite}");
      print("Body: {");
      print("  userId: ${AppSharedPreferences.getUserId()}");
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // Check if response contains the cache path error
        if (response.body.contains("Please provide a valid cache path")) {
                          ...
                          children: [
                            IconButton(
                              ...
                              icon: Icon(
                                ...
                                color: isLiked ? AppColorManager.navy : AppColorManager.white,
                              ),
                            ),
                            SizedBox(
                              width: AppWidthManager.w1Point2,
